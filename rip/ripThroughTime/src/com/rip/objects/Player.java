...
//	public Rectangle hitableBox = new Rectangle(this.getX() + this.getHeight() / 2,this.getY() + this.getWidth() / 2,this.getWidth() / 2,this.getHeight()/2);
	...
	public void handleTime(LevelRenderer lr, Level level, RipGame game) {
		...
		} else if (Gdx.input.isKeyPressed(Keys.SHIFT_LEFT) && getTime() == 100) {
			this.time = 0f;
			this.setUltXY();
			//ultimate
		} else {
	...
	public void dispose() {
		Gdx.app.log(RipGame.LOG, "Player Dispose");
		...
		this.x = this.preultX;
		this.y = this.preultY;
		this.punchSheet.dispose();
		this.punch2Sheet.dispose();
		this.punch3Sheet.dispose();
		this.kick3Sheet.dispose();
		this.kickSheet.dispose();
		this.ULTSheet.dispose();
		this.DEATHSheet.dispose();
